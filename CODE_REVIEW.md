...
## 1. Synthèse de l'Architecture Full-Stack
...
- **Mots de passe sécurisés** : Hachage systématique avec `bcryptjs`.
- **Session HTTP-Only & JWT** : Les jetons de session sont chiffrés avec la bibliothèque moderne `jose` (algorithme `HS256`) et stockés dans un cookie `HttpOnly`, inaccessible au JavaScript malveillant (protection contre les attaques XSS).
...
1. **Gestion de la Pagination** :
   ...
   - *Impact* : Lorsque la base de données contiendra des milliers d'articles, la charge mémoire du serveur augmentera en conséquence. Et la performance en prendra un coup.
2. **Gestion de la Connexion Réseau (Offline / Latence)** :
   - *Constat* : Les Server Actions supposent une connexion Internet stable.
   - *Impact* : En cas de coupure réseau, les messages d'erreur génériques sont affichés.

---

## 4. Recommandations pour la Prochaine Version
